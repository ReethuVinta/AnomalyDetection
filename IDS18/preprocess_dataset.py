...
def main():
    ...
    for filename in data_files:
        if filename == "data/CSVs/02-20-2018.csv":
            ...
            filename == "data/CSVs/02-16-2018.csv"
            or filename == "data/CSVs/02-28-2018.csv"
            or filename == "data/CSVs/03-01-2018.csv"
        ):
            ...
            df = pd.concat(
                [
                    ...
                    reduce_mem_usage(
                        pd.read_csv(
                            filename, low_memory=False, index_col=False, names=columns
                        )
                    ),
                ],
                ignore_index=True,
            ...
            print(
                f"Concatenated dataFrame mem usage: {df.memory_usage().sum()/1024**2} MB",
                flush=True,
            )
            ...
            df = pd.concat(
                [
                    ...
                    reduce_mem_usage(
                        pd.read_csv(
                            filename, low_memory=False, index_col=False, dtype=dtypes
                        )
                    ),
                ],
                ignore_index=True,
