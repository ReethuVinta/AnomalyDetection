...
perm = {'1': [14, 9, 12], '2': [4, 3, 5], '3': [0, 11, 8], '4': [7, 6, 10], '5': [2, 13, 1]}
...
class Conv(nn.Module):
    def __init__(self, num_classes=2, input_size=70,
                 hidden_size=100):
        super().__init__()

        self.conv1d = nn.Conv1d(1,14,kernel_size=3)
        
        self.feature1 = nn.Sequential(nn.Linear(14*68, hidden_size),
...
# log to Tensorboard
tb_logger = TensorboardLogger(f'./tb_data/{cur_time}_cnn1d/')

# log to text file
text_logger = TextLogger(open(f'./logs/{cur_time}_cnn1d.txt', 'w+'))
...
for experience in generic_scenario.train_stream:
    ...
    torch.save(model.state_dict(), './weights/cnn1d/After_training_Task_{}'.format(i))
    print('Model saved!')
