...
class NormalNN(nn.Module):
    ...
    def __init__(self, agent_config):
        ...
        # print("Well multihead is",self.multihead)
    ...
    def create_model(self):
        ...
        model = models.__dict__[cfg['model_type']
                                ].__dict__[cfg['model_name']]()
        ...
        if self.multihead:
            ...
            for t, t_preds in preds.items():
                ...
                if len(inds) > 0:
                    ...
                    loss += self.criterion_fn(t_preds+1e-2, t_target) * len(inds)
    ...
    def learn_batch(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.config['schedule'][-1]):
            ...
            for i, (input, target, task) in enumerate(train_loader):
                ...
                # print('for debugging ',input.shape, target.shape, len(task))
                # self.log(target)
                loss, output = self.update_model(input, target, task)
                
                # print('for debugging ',input)
...
def accumulate_acc(output, target, task, meter):
    if 'All' in output.keys():  # Single-headed model
        ...
        for t, t_out in output.items():
            # The index of inputs that matched specific task
            inds = [i for i in range(len(task)) if task[i] == t]
            if len(inds) > 0:
                t_out = t_out[inds]
