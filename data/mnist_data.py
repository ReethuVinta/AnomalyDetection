...
class MNISTData(Dataset):
    ...
    LABELS='labels.npy'
    DATASET='dataset.npy'
    _MNIST_DATA_DUMP = 'ids_dataset.pickle'
    ...
    def __init__(self, data_path, use_one_hot=False, validation_size=5000):
        ...
        if build_from_scratch:
            ...
            path_data=os.path.join(data_path,MNISTData.DATASET)
            path_label=os.path.join(data_path,MNISTData.LABELS)


            # read labels
            all_labels = MNISTData._read_labels(path_label)
            # train_labels = MNISTData._read_labels(label)
            # test_labels = MNISTData._read_labels(test_labels_fn)

            # read images
            all_inputs = MNISTData._read_images(path_data)
            # train_inputs = MNISTData._read_images(train_images_fn)
            # test_inputs = MNISTData._read_images(test_images_fn)

            train_inputs, test_inputs, train_labels, test_labels = train_test_split(all_inputs, all_labels, test_size=0.2, random_state=42)
