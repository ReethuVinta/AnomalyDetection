# Continual-Learning-Algorithms on IDS Dataset
...
```
Before running this processdataset.py make sure you have *.csv file of [IDS dataset](https://www.kaggle.com/cicdataset/cicids2017) downloaded in the same directory.
> To get IDS dataset run processdataset.py so that you can get *.npy files in data/ directory.
```bash
...
- Example: Run all algorithms in the incremental domain scenario with split IDS.
```bash
./scripts/split_IDS_incremental_domain.sh 0
