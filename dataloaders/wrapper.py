...
class CacheClassLabel(data.Dataset):
    ...
    def __init__(self, dataset):
        ...
        label_cache_filename = path.join(
            './data', str(type(dataset))+'_'+str(len(dataset))+'.pth')
        ...
        print("Number of unique classes are --> ", self.number_classes)
...
class AppendName(data.Dataset):
    ...
    def __getitem__(self, index):
        features, target = self.dataset[index][0], int(self.dataset[index][1])
        target = target + self.first_class_ind
        return features, target, self.name
...
class Subclass(data.Dataset):
    ...
    def __init__(self, dataset, class_list, remap=True):
        ...
        assert isinstance(
            dataset, CacheClassLabel), 'dataset must be wrapped by CacheClassLabel'
        self.dataset = dataset
    ...
    def __getitem__(self, index):
        features, target = self.dataset[self.indices[index]]
        if self.remap:
